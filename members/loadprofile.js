...
function displayMemberProfile(json) {
    ...
    if (json.articles.length > 0) {
        ...
        for (var i = 0; i < json.articles.length; i++) {
            list.innerHTML += `<button class="item" onclick="window.location.href=/articles/articles.html?id=${json.articles[i].id}"><h3>${json.articles[i].name}</h3><p>${json.articles[i].body}</p></button>`
        }
    }
}